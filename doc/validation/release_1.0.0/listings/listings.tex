...
\newcommand{\topic}{Listings}
\newcommand{\testinglog}{listings-testing-log.pdf}
...
The application includes numerous listings for summarizing model development and evaluation, including:

\begin{itemize}
  \item Concentration vs Time Multipanel plots
  \item Subject exclusions tables
  \item Observation exclusions tables
  \item Text file listing from disk
\end{itemize}

For all of these objects, all inputs are checked to ensure that the the output is reactive to the input.  Screenshots are used to document evidence of this.

{\bf Figure from disk}

The user may alternatively point to a file on disk to be included in the output RTF file.  In this step, a PNG figure is read from disk and it is verified that a preview is displayed to the user.  Evidence is again documented via screenshot.
